...
class NormalGridLock implements Generator<Car> {
    ...
    private void search(List<Choice> choiceSet, String dir, String facing, int size, Coordination co) {
        ...
        for(Choice c:choiceSet) {
            if (c.direct.equals(dir) && c.facing.equals(facing) && c.size == size) {
                c.critical.add(co);
                test = false;
            }
        }
    ...
    private Set<Coordination> generateCar(Coordination target, int[][] board, List<Car> entities) {
        ...
        for (int i = 2; i <= 3; i++) {
            ...
            if (flag) {
                ...
                if (validate(board, target, "Tail", i, dir)) search(choiceSet, dir, "Tail", i, critical);
                if (validate(board, target, "Head", i, dir)) search(choiceSet, dir, "Head", i, critical);
            ...
            if (i == 3) {
                ...
                if (target.getY() + 3 < board.length && board[target.getY() + 3][target.getX()] == 0) {
                    for (int j = target.getY(); j <= target.getY() + 3; j++) {
                        if (board[j][target.getX()] != 0) {
                            flag = false;
                            break;
                        }
                ...
                if (flag) {
                    ...
                    if (validate(board, target, "Body", i, dir)) search(choiceSet, dir, "Body", i, critical);
                }
            ...
            for (int i = 2; i <= 3; ++i) {
                ...
                if (i == 3) {
                    ...
                    if (target.getX() + 3 < board.length && board[target.getY()][target.getX()+3] == 0) {
                        for (int j = target.getX(); j <= target.getX() + 3; j++) {
                            if (board[target.getY()][j] != 0) {
                                flag = false;
                                break;
                            }
